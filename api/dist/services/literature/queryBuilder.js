...
exports.buildLiteratureQueries = void 0;
...
const buildPhrase = (tokens) => {
    const filtered = tokens.map((token) => cleanToken(token)).filter((token) => Boolean(token));
...
const scoreClaim = (claim) => {
    ...
    let bonus = 0;
    if (/diagnos|אבח|finding/.test(type))
        bonus += 1.5;
    if (/procedure|surgery|operation|imaging|test|biopsy/.test(type))
        bonus += 1;
    return base + bonus;
};
const buildClaimQuery = (claim) => {
...
const timelinePriority = (type) => {
    const normalized = normalizeText(type ?? '');
    if (/surgery|operation|procedure/.test(normalized))
        return 3;
    if (/hospital|admission|discharge/.test(normalized))
        return 2;
    if (/follow|clinic|review/.test(normalized))
        return 1;
    return 0;
};
const buildLiteratureQueries = ({ claims = [], timeline = [] }) => {
...
exports.buildLiteratureQueries = buildLiteratureQueries;
