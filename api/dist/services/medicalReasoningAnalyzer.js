...
exports.analyzeMedicalReasoning = void 0;
const medicalSpecialtyRules_1 = require("./medicalSpecialtyRules");
const isSamePeriod = (dateA, dateB) => {
    if (!dateA || !dateB)
        return false;
    return dateA.slice(0, 7) === dateB.slice(0, 7);
};
const hasKeyword = (text, keywords) => keywords.some((keyword) => text.toLowerCase().includes(keyword));
const LEGAL_DISCLAIMER = 'המערכת אינה מחליפה מומחה רפואי. זהו כלי תומך החלטה משפטית בלבד.';
const createReasoningFinding = ({ code, message, severity, relatedClaimIds, domain, basisHint, missingHint, assertionType, reliabilityLevel, }) => {
    const defaultBasis = relatedClaimIds && relatedClaimIds.length
...
const analyzeMedicalReasoning = (claims, timeline) => {
    const findings = [];
        ...
        disabilityClaims.forEach((disabilityClaim) => {
            if (isSamePeriod(workClaim.date, disabilityClaim.date)) {
                findings.push(createReasoningFinding({
                    ...
                    severity: 'warning',
                    relatedClaimIds: [workClaim.id, disabilityClaim.id].filter(Boolean),
                    domain: 'GENERAL',
    ...
    treatmentEvents.forEach((event) => {
        const hasFollowUp = followupEvents.some((follow) => follow.date && event.date && new Date(follow.date).getTime() > new Date(event.date).getTime());
        if (!hasFollowUp) {
            findings.push(createReasoningFinding({
                code: 'TREATMENT_GAP',
    ...
    const specialtyFindings = (0, medicalSpecialtyRules_1.runSpecialtyRules)(claims, timeline);
    return [...findings, ...specialtyFindings];
};
exports.analyzeMedicalReasoning = analyzeMedicalReasoning;
