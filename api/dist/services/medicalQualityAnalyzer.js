...
exports.analyzeMedicalQuality = void 0;
...
const computeTimelineGaps = (events) => {
...
const buildQualityFinding = ({ code, message, severity, relatedClaimIds, domain, basisHint, missingHint, assertionType, reliabilityLevel, }) => {
    const basis = [basisHint ?? 'מדדי איכות מסמך דטרמיניסטיים'];
...
const analyzeMedicalQuality = ({ claims, timeline, flags, reasoningFindings = [] }) => {
    const safeClaims = Array.isArray(claims) ? claims : [];
    const safeTimeline = Array.isArray(timeline) ? timeline : [];
    const safeFlags = Array.isArray(flags) ? flags : [];
    const findings = [];
    if (!safeClaims.length) {
        findings.push(buildQualityFinding({
    ...
    if (percentWithDates < 0.6) {
        findings.push(buildQualityFinding({
            ...
            relatedClaimIds: safeClaims.filter((claim) => !isValidDate(claim.date)).map((claim) => claim.id).filter(Boolean),
    ...
    if (percentLowEvidence > 0.3) {
        findings.push(buildQualityFinding({
            ...
            relatedClaimIds: lowEvidenceClaims.map((claim) => claim.id).filter(Boolean),
            basisHint: 'מדד: שיעור קביעות עם איכות ראיה נמוכה',
            missingHint: 'יש לצרף מקורות סרוקים איכותיים או אישור ידני',
        }));
    }
    const disabilityClaims = safeClaims.filter((claim) => /(disability|נכות|%)/i.test(`${claim.type ?? ''} ${claim.value ?? ''}`));
    ...
    if (distinctValues.length > 1) {
        findings.push(buildQualityFinding({
            ...
            severity: 'warning',
            relatedClaimIds: disabilityValues.map((entry) => entry.id).filter(Boolean),
...
exports.analyzeMedicalQuality = analyzeMedicalQuality;
