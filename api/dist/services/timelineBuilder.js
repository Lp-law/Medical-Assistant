...
exports.buildTimelineFromClaims = void 0;
const crypto_1 = require("crypto");
const ADVANCED_TYPE_RULES = [
...
const getSubject = (type) => SUBJECT_GROUP[type] ?? type;
...
const parseDateFromClaim = (claim) => {
...
const normalizeTimestamp = (value, precision) => {
    if (!value)
        return undefined;
    if (precision === 'day') {
...
const shouldGroupEvents = (prev, current) => {
...
const isGenericDescription = (description, type) => {
...
const attachHiddenReferences = (events) => {
    const visible = events.filter((event) => !event.hidden);
    if (!visible.length) {
        return;
    }
    const findNearest = (event) => {
...
const buildTimelineFromClaims = (claims = []) => {
    const flags = [];
    const baseEvents = claims.map((claim, index) => {
        ...
        return {
            id: claim.id ?? (0, crypto_1.randomUUID)(),
            date: parsed.value,
    ...
    const hasUndatedBetween = (prev, next) => {
        const prevIndex = Math.min(...prev.orderIndexes);
        const nextIndex = Math.min(...next.orderIndexes);
        return baseEvents.some((event) => event.sortTimestamp === undefined &&
    ...
    const finalEvents = combinedEvents.map((event) => ({
        id: event.id,
        date: event.date,
...
exports.buildTimelineFromClaims = buildTimelineFromClaims;
