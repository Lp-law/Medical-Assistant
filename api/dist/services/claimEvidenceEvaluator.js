...
exports.evaluateClaimEvidence = void 0;
const qualityOrder = ['high', 'medium', 'low'];
const downgrade = (current, steps = 1) => {
...
const hasTraceability = (source) => {
    ...
    if (source.lineRange && source.lineRange.length === 2)
        return true;
    if (source.snippet && source.snippet.trim().length > 8)
        return true;
    return false;
};
const trimSnippet = (snippet) => {
    if (!snippet)
        return undefined;
...
const determineAssertionType = (quality, hasFullTrace) => {
    if (quality === 'high' && hasFullTrace)
        return 'FACT';
    if (quality === 'medium')
        return 'INTERPRETATION';
    return 'POSSIBILITY';
};
const evaluateClaimEvidence = ({ claims, ocrScore, lexicalText }) => {
...
exports.evaluateClaimEvidence = evaluateClaimEvidence;
