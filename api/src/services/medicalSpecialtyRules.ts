...
const hasSupportingClaim = (claims: Claim[], keywords: string[]): boolean => findClaims(claims, keywords).length > 0;

const hasTimelineEvent = (timeline: TimelineEvent[], keywords: string[]): boolean =>
  timeline.some(
    (event) => hasKeyword(normalize(event.type), keywords) || hasKeyword(normalize(event.description), keywords),
  );
...
const enrichFinding = (finding: SpecialtyFinding): SpecialtyFinding => {
...
const filterClaims = (claims: Claim[], include: string[], exclude: string[] = []): Claim[] =>
  findClaims(claims, include).filter((claim) => {
...
export const runSpecialtyRules = (claims: Claim[], timeline: TimelineEvent[]): SpecialtyFinding[] => {
  ...
  const hasTimelineKeywords = (keywords: string[]): boolean => hasTimelineEvent(safeTimeline, keywords);
