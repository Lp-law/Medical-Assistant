...
const TAG_PATTERNS: { label: string; regex: RegExp }[] = [
...
export const deriveRules = (text: string): string[] => {
  const lines = text
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  const keywordRules = lines.filter(
    (line) => /(יש\s+ל|חובה|נדרש|מומלץ|אסור)/.test(line) && line.length > 20 && line.length < 240
  );
...
export const buildInsights = (text: string, tags: string[]): Insight[] => {
  const confidence: Insight['confidence'] = tags.length > 3 ? 'High' : tags.length > 1 ? 'Medium' : 'Low';
  return [
    {
      title: 'תגיות עיקריות',
