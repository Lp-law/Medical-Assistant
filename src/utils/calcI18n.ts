...
const dict: Record<Lang, Record<string, string>> = {
  he: {
    ...
    helpActions: 'פעולות: הוסף ראש נזק, אשם תורם (%), מל״ל (סכום), פגיעה בסיכויי החלמה (%), הוסף נתבע – כולם מוסיפים שורה או הפחתה בטבלה.',
    helpTemplates: 'תבניות: צפייה בספריית תבניות או שמירת המחשבון הנוכחי כתבנית.',
    helpHistory: 'היסטוריה: בטל / בצע שוב לשינויים אחרונים.',
    helpExport: 'ייצוא: ייבוא JSON, ייצוא אקסל/JSON/DOCX/Word.',
    helpTools: 'כלים: איפוס, שאלון חכם.',
    helpFormula: 'נוסחת החישוב: סה״כ נטו (מראשי נזק) → מינוס אשם תורם (%) → מינוס מל״ל (סכום ₪) → מינוס פגיעה בסיכויי החלמה (%) → חלוקת נתבעים. סה״כ לתשלום = נטו לאחר הפחתות + שכ״ט עו״ד (%) + הוצאות תובע (₪).',
  },
  'en-GB': {
    ...
    helpActions: 'Actions: Add damage head, Contributory negligence (%), NII (amount), Loss of chance (%), Add defendant – each adds a row or reduction to the table.',
    helpTemplates: 'Templates: Open template library or save current sheet as template.',
    helpHistory: 'History: Undo / Redo for recent changes.',
    helpExport: 'Export: Import JSON, Export Excel/JSON/DOCX/Word.',
    helpTools: 'Tools: Reset, Smart Questionnaire.',
    helpFormula: 'Formula: Net total (from damage heads) → minus contributory negligence (%) → minus NII (amount ₪) → minus loss of chance (%) → defendants split. Grand total = net after reductions + attorney fee (%) + plaintiff expenses (₪).',
  },
};
